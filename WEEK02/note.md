...
8.使用内存映射文件（memory‑mapped file）可以把磁盘上的大数组当作普通的 NumPy 数组来访问，而实际只把被访问的那一部分数据页载入物理内存。这样即使文件远大于可用内存，也能通过切片（slice）高效地读取或写入所需的数据块，而不会一次性把整个文件读进内存。

9.deque 底层是双向链表（实际是固定大小块的双向链表，兼顾缓存效率）。每个节点持有指向前后节点的指针，在两端插入/删除只需修改几个指针，O(1) 完成，与队列长度无关。  
