### 切片
[]本质是：Python 看到 a[i, j] 时，不是把它当成两个独立参数，而是会把 i 和 j 组成一个元组，再交给 __getitem__ 处理，所以它等价于 a.__getitem__((i, j))) 这一类调用。  
普通切片 seq[start:stop:step] 本质上也会转换成把一个 slice 对象传给 __getitem__ 的调用。  
numpy里的ndarray也用这套机制，所以它的多维切片和多维多维索引同。  


python里，...是一个真实对象，内置对象Ellipsis，代表“其余维度全部照选”。  
...
我们知道，+=和*=属于增量赋值运算符，它们是否原地修改对象取决于左操作数的类型。  
